! function() {
        ...
        bP.bars = function(mb) {
            ...
            function calculateSubBars(part) {
                ...
                ps.forEach(function(d) {
                    var g = map.get(d.key);
                    var bars = bpmap(d.values, 0, 0, _or == "vertical" ? g.y - g.height : g.x - g.width, _or == "vertical" ? g.y + g.height : g.x + g.width);
    ...
    viz.ch = function() {
        ...
        function ch(_) {
            ...
            _.each(function() {
                ...
                labels.filter(function(d) { return d.type == "g" })
                    ...
                    .attr("transform", function(d) {
                        if ((d.angle > Math.PI / 2 && d.angle < Math.PI) || (d.angle > (Math.PI + Math.PI / 2) && d.angle < Math.PI * 2)) {
                            return "rotate(" + (d.angle * 180 / Math.PI) + ")" +
                                "translate(" + (innerRadius + 27) + ")" + (d.angle > Math.PI ? "rotate(0)" : "rotate(-180)");;
                        } else {
                            return "rotate(" + (d.angle * 180 / Math.PI) + ")" +
                    ...
                    .text((d) => {
                        return `${d.source}`; // `${d.source} (${d.value})`
                    })
                    .each(function(d) { this._current = d; });
        ...
        function relayouts(fixedSource) {
            ...
            var m = d3.map(newchords.map(function(d) { return { startAngle: d.startAngle, endAngle: d.endAngle, indexsubindex: d.indexsubindex }; }), function(d) { return d.indexsubindex; });
    ...
    function viz_arc(x) {
        ...
        return ["M", ss, "A", x[0], x[0], "0", (x[3] - x[2] > π ? 1 : 0), "1", se, "L", ee,
            "A", x[1], x[1], "0", (x[3] - x[2] > π ? 1 : 0), "0", es, "z"
        ].join(" ");
    }
